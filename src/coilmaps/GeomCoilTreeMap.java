...
/* GeomCoilTreeMap.java (created on Aug 3, 2008) */
 ...
 * @author Angus Forbes
 */
public class GeomCoilTreeMap extends GeomRect
{
  ...
  final public long cutSpeed = 5000L; //100L;
  final public long cutSpeedWhenReforming = 200L; //100L;
  final public long cutPause = 50L;
   ...
   */
  public GeomCoilTreeMap(float x, float y, float z, float w, float h,
  ...
  /**
   * Adds a city to a leaf node and increments the event count of the leaf node and all of its
   * ancestors.
   * @param cityData The city event to add.
   * @param node The leaf node it is being added to.
   */
  public void addCityToNode(CityData cityData, CoilNode node)
  {
    node.cities.add(cityData);

    //also need to explicitly add cityData's Geoms! (textGeom and cityGeom)
    addGeom(cityData.textGeom, false);
    addGeom(cityData.cityGeom, false);
  }

  public void removeCitiesFromNodes(List<CoilNode> nodes)
  {
    for (CoilNode node : nodes)
    {
      for (CityData cityData : node.cities)
      {
        if (cityData.textGeom != null)
        {
          removeGeom(cityData.textGeom);
        }
        if (cityData.cityGeom != null)
        {
          removeGeom(cityData.cityGeom);
        }
      }
    }
  }
...
//      }
    ...
    for (CoilNode node : nodes)
    {
      if (node.cut != .5f)
      {
        ...
        Behavior bcc = BehaviorCoilCut.coilCut(node, 
          stTime, 
          cutSpeedWhenReforming, .5f - node.cut);
        this.attachBehavior(bcc);
   ...
   * @return
   */
  public static GeomCoilTreeMap findCoilTreeUsingLngLat(float lng, float lat, GeomCoilTreeMap... trees)
  {
    for (GeomCoilTreeMap gct : trees)
    {
  ...
  public void draw(GL gl, GLU glu, float offset)
  {
    ...
    if (drawRects)
    {
      ...
      for (CoilNode node : leafNodes)
      {
        ...
        //System.out.println("p1 : " + node.col + "/" + node.row);
        //System.out.println("p2 : " + (node.col + node.w) + "/" + node.row);
        //System.out.println("p3 : " + (node.col + node.w) + "/" + (node.row + node.h));
        //System.out.println("p4 : " + node.col + "/" + (node.row + node.h));
        ...
        if (drawCities == true && isReforming == false)
        {
          //gl.glColor4fv(CebProperties.cityColor.array(), 0);

          //gl.glColor4f(1f,.56f,.1f,1f);
          for (CityData city : node.cities)
          {
            float cx = node.x + (node.w * city.xperc);
            float cy = node.y + (node.h * city.yperc);
            float inc = .015f;

            city.cityGeom.setPos(cx, cy, 0f);
            city.cityGeom.isActive = true;
            ...
            if (drawNames == true)
            {
              ...
              //System.out.println("cx/cy for city = " + cx + "/" + cy);
              city.textGeom.setPos(cx - city.textGeom.w * .5f, cy + .02f, 0f);
              city.textGeom.isActive = true;

              //city.textGeom.anchor.x = 2f;
              //city.textGeom.anchor.y = 2f;
              //city.textGeom.draw(gl, glu, .01f);
            }
            else
            {
              //city.textGeom.isActive = false;

            }
          /*
          gl.glBegin(gl.GL_QUADS);
          
          gl.glVertex3f(cx - inc, cy - inc, offset);
          gl.glVertex3f(cx + inc, cy - inc, offset);
          gl.glVertex3f(cx + inc, cy + inc, offset);
          gl.glVertex3f(cx - inc, cy + inc, offset);
        ...
        else //not drawing cities, lets make sure they are turned off...
        {
          for (CityData city : node.cities)
          {
            city.cityGeom.isActive = false;
            city.textGeom.isActive = false;
          }

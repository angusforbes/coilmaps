...
//  };
  ...
  public void init()
  {
    ...
    this.rootNode = new CoilNode(0, null, 0f, 0f, w, h, h * .1f, h * .5f, 0f, 360f);
  ...
  private void updatePositions(CoilNode parent)
  {
    ...
    //HORIZONTAL = spoke //circle
    //VERTICAL = circle //spoke
    switch (parent.dir)
    {
      case VERTICAL:
        parent.side1.setArc(
          parent.internalRadius,
          parent.internalRadius + ((parent.externalRadius - parent.internalRadius) * parent.cut),
          parent.startAngle,
          parent.angleWidth);
        parent.side2.setArc(
          parent.internalRadius + ((parent.externalRadius - parent.internalRadius) * parent.cut),
          parent.externalRadius,
          parent.startAngle,
          parent.angleWidth);
      ...
      case HORIZONTAL:
        parent.side1.setArc(
          ...
          parent.externalRadius,
          parent.startAngle,
          parent.angleWidth * parent.cut);
        parent.side2.setArc(
          parent.internalRadius,
          parent.externalRadius,
          parent.startAngle + (parent.angleWidth * parent.cut),
          parent.angleWidth * (1f - parent.cut));
   ...
   * @param parent
   */
  private void updatePositionsSquare(CoilNode parent)
  {
    if (parent.isLeaf == true)
    {
      return;
  ...
  {
    //get angle (where straight up = 0f) and radius from center of screen.
   ...
   * @return A leaf node.
   */
  public CoilNode findLeafNodeArc(float radius, float angle)
  {
    return findLeafNodeArc(rootNode, radius, angle);
  }
   ...
   * @return A leaf node.
   */
  public CoilNode findLeafNodeArc(CoilNode node, float radius, float angle)
  {
    //get angle (where straight up = 0f) and radius from center of screen.
    ...
    switch (node.dir)
    {
      case VERTICAL:
        //System.out.print("is col (" + col + ") < " + node.side2.col + "? ");
        if (radius < node.side2.internalRadius)
        {
          //System.out.println("...yes, use side1 ");
          which = node.side1;
        }
      ...
      case HORIZONTAL:
        //System.out.print("is row (" + row + ") < " + node.side2.row + "? ");
        if (angle < node.side2.startAngle)
        {
          //System.out.println("...yes, use side1 ");
          which = node.side1;
        }
    ...
    {
      return findLeafNodeArc(which, radius, angle);
    }
  }
  ...
  public void draw(GL gl, GLU glu, float offset)
  {
    ...
    gl.glTranslatef(w * .5f, h * .5f, 0f);

    GLUquadric quadric = BehaviorismDriver.renderer.quadricRenderer;
    ...
    for (CoilNode node : cutNodes)
    {
      //if (node.textureData != null)
      {
        ...
        if (node.externalRadius - node.internalRadius < h * .1f)
        {
          //continue;
        }
        if (node.angleWidth < 15f)
        {
          continue;
        }
        ...
        //glu.gluPartialDisk(quadric, .5f, 1f, 160, 160, -30f, 90f);
        glu.gluPartialDisk(quadric,
          node.internalRadius, node.externalRadius,
          node.slices, node.rings,
          node.startAngle, node.angleWidth);
      //160, 160, -30f, 90f);
  ...
  {
    float cx = BehaviorismDriver.mouseListener.mouseGeom.x;
    float cy = BehaviorismDriver.mouseListener.mouseGeom.y;

    final float[] polar = getDistAndAngleFromMouse(cx, cy);
    ...
    Thread t = new Thread()
    {
      ...
      public void run()
      {
        CoilNode leaf = findLeafNodeArc(polar[0], polar[1]);
          //BehaviorismDriver.mouseListener.mouseGeom.x, BehaviorismDriver.mouseListener.mouseGeom.y);
  ...
  private float[] getDistAndAngleFromMouse(float cx, float cy)
  {
    System.out.println("cx/cy = " + cx + "/" + cy);
    Point3f mP = new Point3f(cx, cy, 0f);
    Point3f cP = new Point3f(w * .5f, h * .5f, 0f);

    final float radius = GeomUtils.getDistanceBetweenPoints(mP, cP);
    float ang = (float) (Math.toDegrees(GeomUtils.getAngleBetweenPoints(cP, mP)) - 90f) * -1f;
    if (ang < 0f)
    {
      ang += 360f;
    }

    final float angle = ang;

    return new float[]{radius, angle};
  }
